    ...
    <form action="{{ route('karyawan.update', ['id' => $karyawan->id]) }}" method="POST" class="mx-auto p-6">
        ...
        <input type="hidden" name="_method" value="PUT">
         ...
         <div class="mb-5">
            ...
            <input type="text" name="id" id="id" placeholder="Masukkan ID karyawan" class="w-full px-4 py-2 border border-blue-300 rounded-md"
            value="{{ $karyawan->id }}" readonly>

        <div class="mb-5">
            <label for="nama" class="block text-sm font-medium text-slate-700 mb-2">Nama</label>
            <input type="text" name="nama" id="nama" placeholder="Masukkan nama karyawan" class="w-full px-4 py-2 border border-blue-300 rounded-md"
            value="{{ $karyawan->nama }}" required>
        </div>

        <div class="mb-6 mt-2">
            <label for="posisi" class="block text-sm font-medium text-slate-700 mb-2">Posisi</label>
            <input type="text" name="posisi" id="posisi" placeholder="Masukkan posisi karyawan" class="w-full px-4 py-2 border border-blue-300 rounded-md duration-200"
